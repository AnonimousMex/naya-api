...
from app.api.test.test_schema import (
    AnswersRequest,
    AnswersResponse,
    EmotionPercentageResponse,
    ListTestRequest,
    PostAnswerRequest,
    TestDetailsReponse,
    TestDetailsRequest,
...
@test_router.get("/init-test", response_model=NayaResponseModel[TestStoriesResponse])
async def get_situtations(
    session: SessionDep, token: str = Header(..., alias="Authorization")
):
    try:
...
@test_router.post(
    "/list-answers", response_model=NayaResponseModel[List[AnswersResponse]]
)
...
# test_router.post("/percentage-answers") - is imporant to calculate the percentage of each emotion in the answers of the test
@test_router.post("/test-details", response_model=NayaResponseModel[TestDetailsReponse])
...
@test_router.post("/list-test", response_model=NayaResponseModel[List[TestsResponse]])
async def list_test(session: SessionDep, request: ListTestRequest):
    try:
        test_controller = TestController(session=session)
